...
def run_daily_pipeline(send_email_now: bool = False) -> dict[str, Any]:
    ...
    if activities:
        ...
        _PRESERVE = {
            "epoc", "calories_kcal", "trimp", "tss", "training_stress_score", "segments", "hr_timeseries",
...
@app.route("/activity/<activity_id>/label", methods=["POST"])
def save_label(activity_id):
    label = (request.json or {}).get("label", "").strip()
    db = load_metrics()
    for day_acts in (db.get("activities") or {}).values():
        for a in day_acts:
